...
{      
    ...
    TYPE tmperr, parterr, gamma2p, discr, accum = 0;
    ...
    if (TADAPTIVE) {
        for(uint16_t kk = 0; kk < K; kk++) { 
            ...
            accum += parterr+0.5*p_gamma_1[gm_ind];
        }
        gamma2p = C[K*imsize+tind] - p_opt[3]*(accum);
        discr = pow(gamma2p,2) + 4*p_opt[3]*(valK[tind]+p_gamma_2-1);
        C[K*imsize+tind] = 0.5*(gamma2p+sqrt(discr));
    }
